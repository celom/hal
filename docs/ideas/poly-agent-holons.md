...
- **What works the cycle** — the agent authoring implementations, proposals, logbook entries. The poly-agent axis.
...
5. **Differential eval-gaming** — agents differ in willingness to overfit an eval rather than satisfy intent. The protocol's answer is more/better evals, but the *rate* should be tracked per agent in the logbook.
