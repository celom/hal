...
> Frozen as provenance. The living layer: [canon](../canon.md) · [notebook](../notebook/README.md).
...
One program, three layers. The five variations that survived divergence (canon, code-as-lockfile, steady-state loop, seam-first, no-orchestrator) collapse into:
...
- **R6 — Two valid outcomes.** A cycle returns either (a) an implementation passing evals, or (b) a **renegotiation**: a proposed change to the holon's own intent/contract/evals with rationale, escalated to the parent. Discovering the spec is wrong is a success outcome.
- **R7 — Steady state by convention.** Every cycle begins by running the full eval suite. Any red eval outranks new intent work. Intents no longer referenced by any parent trigger a deletion review.
...
- Canon v0 committed to this repo (this document seeds it).
...
- By Dec 2026: **≥30 logged cycles, ≥5 drills, a seam census worth citing, and canon v1 amended with citations to notebook entries** — plus an honest verdict on the bundle hypothesis.
...
Refined 2026-07-20 from the raw hypothesis: "each cycle should focus on small deliverables defined by well-defined inputs, outputs and evals, autonomous 'molecules' of logic that subsist while useful but are easily replaced." Divergence produced six variations: (1) bundle canon, (2) code-is-the-lockfile, (3) steady-state loop, (4) seam-first, (5) written-for-agents, (6) no-orchestrator. Converged on 1+6 as spine, 3 as the original contribution, 4 as the expected pain, 2 as the sharpest experiment; 5 deferred. Key crack to watch: intent is discovered by building — R6 (renegotiation as a valid outcome) is the designed answer; watch whether it carries the load.
