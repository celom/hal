...
The holon bundle gains a `runtime` declaration (language, build command, invocation surface). Context budget rule (R5) is unchanged — the agent loads the bundle, the implementation, and dependencies' *contracts*. It never needs a dependency's language.
